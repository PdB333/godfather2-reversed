// FUNC_NAME: VectorContainer::reserve
void VectorContainer::reserve(uint newCapacity)
{
  int iVar1;
  ...
  if ((uint)thisPtr[2] < newCapacity) {
    newBuffer = (undefined4 *)FUN_009c8e80(newCapacity * 8); // allocate 8 bytes per element
    if (*thisPtr != 0) {
      i = 0;
      srcPtr = newBuffer;
      if (thisPtr[1] != 0) {
      ...
      FUN_009c8f10(*thisPtr); // free old buffer
    }
